...
.footnote[
[1] Another popular IDE that works nicely with `R` and `Git` is [*Visual Studio Code*](https://code.visualstudio.com/) - or VSCode for short - by *Microsoft*.
...
```{r rstudio-ssh, out.width = "75%", echo=FALSE}
...
```{r gitlab-ssh, out.width = "60%", echo=FALSE}
...
Which URL you should enter depends on the authentication method you use. If you use `HTTPS`, you can simply copy the URL from the address bar of your browser.
...
```{r gl-ssh, out.width = "70%", echo=FALSE}
...
```{r staged, out.width = "70%", echo=FALSE}
...
```{r commit, out.width = "75%", echo=FALSE}
