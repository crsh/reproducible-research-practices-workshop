...
```{r binger-img, out.width = "75%", echo=FALSE}
...
Using a *BinderHub* deployment like [*mybinder.org*](https://mybinder.org/) or [*GESIS Notebooks*](https://notebooks.gesis.org/binder/) you can turn an existing public `Git` repository into an executable environment.
...
In the `runtime.txt` file, you need to specify a date indicating which *CRAN* snapshot to use from the [*CRAN Time Machine*](https://mran.microsoft.com/timemachine) which is part of the [*Microsoft R Application Network* (MRAN)](https://mran.microsoft.com/). This is used to install `R` and *CRAN* packages. For example: `r-2022-04-28` (optionally, you can also specify a specific `R` version to be installed: `r-4.1-2022-04-28`).

---

# Binderizing your `Git` repository

In the `install.R` file, you need to specify which `R` packages to install as you normally would in an `.R` or `.Rmd` file (e.g., `install.packages(c("gapminder", "tinytex"))).

--

There are many more options for changing or extending the executable environment. Two good resources to learn more are the [*Turing Way* guide *Zero-toBinder*](https://the-turing-way.netlify.app/communication/binder/zero-to-binder.html) or the [*Binder* example for `R`](https://github.com/binder-examples/r). 
...
3. One thing you need to consider is how to share your data when using *Binder*. The [*Turing Way* guide *Zero-toBinder*](https://the-turing-way.netlify.app/communication/binder/zero-to-binder.html) has some suggestions for that.
4. By default, the link to the executable environment will open an instance of [*Jupyter Notebook*](https://jupyter.org/), but it is also possible to run [*Jupyter Lab*](https://jupyterlab.readthedocs.io/en/stable/#), *RStudio* or *R Shiny* (the easiest way to change this is by adding parameters at the end of the URL, such as `$urlpath=rstudio`). 
]
...
We have prepared a Binderized repo for you that you can test, clone, fork or whatever else you would like to to: https://github.com/jobreu/demo-repo
...
When working with `R Markdown`, you can create a `packages.bib` file to cite the packages you have used or use the [`grateful` package](https://pakillo.github.io/grateful/).
