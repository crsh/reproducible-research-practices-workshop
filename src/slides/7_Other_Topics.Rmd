...
```{r binger-img, out.width = "100%", echo=FALSE}
...
In the `install.R` file, you need to specify which `R` packages to install as you normally would in an `.R` file (e.g., `install.packages(c("gapminder", "tinytex"))). *CRAN* ackages are installed through the [*R Studio Package Manager*](https://packagemanager.rstudio.com/client/#/).
